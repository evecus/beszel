...
import { $authenticated, $copyContent, $direction, $publicKey, $userSettings, $selfMode, $selfHasAuth } from "@/lib/stores.ts"
import * as systemsManager from "@/lib/systemsManager.ts"

const LoginPage = lazy(() => import("@/components/login/login.tsx"))
const SelfLoginPage = lazy(() => import("@/components/login/self-login.tsx"))
...
const App = memo(() => {
	...
	useEffect(() => {
		...
		// check if running in self-monitoring mode
		pb.send("/api/beszel/self-info", {}).then((data) => {
			if (data?.selfMode) {
				$selfMode.set(true)
				$selfHasAuth.set(!!data.hasAuth)
			}
		}).catch(() => {
			// not self-mode or endpoint not available, ignore
		})
		// get user settings
		updateUserSettings()
...
const Layout = () => {
	...
	const selfMode = useStore($selfMode)
	const selfHasAuth = useStore($selfHasAuth)

	return (
		<DirectionProvider dir={direction}>
			{!authenticated ? (
				<Suspense>
					{selfMode && !selfHasAuth ? (
						<SelfLoginPage />
					) : selfMode && selfHasAuth ? (
						<SelfLoginPage requirePassword />
					) : (
						<LoginPage />
					)}
				</Suspense>
