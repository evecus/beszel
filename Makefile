...
# ──────────────────────────────────────────────────────────────────────
# Build single unified binary  (hub + agent in one executable)
#
#   ./beszel          → self-monitoring (hub + local agent, no extra args)
#   ./beszel -hub     → hub-only  (web dashboard for remote agents)
#   ./beszel -agent   → agent-only (monitored server, connects to a hub)
# ──────────────────────────────────────────────────────────────────────
build-combined: tidy $(if $(filter false,$(SKIP_WEB)),build-web-ui)
	GOOS=$(OS) GOARCH=$(ARCH) go build $(AGENT_GO_TAGS) \
		-o ./build/beszel$(EXE_EXT) \
		-ldflags "-w -s" \
		./internal/cmd/combined

build-combined-dev: tidy
	mkdir -p ./internal/site/dist && touch ./internal/site/dist/index.html
	GOOS=$(OS) GOARCH=$(ARCH) go build -tags development $(AGENT_GO_TAGS) \
		-o ./build/beszel$(EXE_EXT) \
		-ldflags "-w -s" \
		./internal/cmd/combined
